...
for nP = 1:length(particles.x)
    particles.weights.velocity(nP) = normpdf(particles.velocity(nP),velocity,variance.velocity);
end
sumWeights = sum(particles.weights.velocity);
particles.weights.velocity = particles.weights.velocity/sumWeights;

for nP = 1:length(particles.x)
    particles.weights.angularRate(nP) = normpdf(particles.angularRate(nP),angularRate,variance.angularRate);
end
