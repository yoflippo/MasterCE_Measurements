...
nm.syncPoints = 'syncPoints_INTERPOLATED.mat';
nm.cutPoints = 'cutPoints_INTERPOLATED.mat';
...
for nF = 1:length(files.wmpm)
    ...
    try
        ...
        sUWB = applyLarssonToUWBwithOptitrack_INTERPOLATED(sOptitrack,files.uwb(nF));
...
f = f(contains({f.name},'RS_','IgnoreCase',true));
f = f(contains({f.name},'~','IgnoreCase',true));
f = f(contains({f.name},'_results_solver','IgnoreCase',true));
f = makeFullPathFromDirOutput(f);
end
