...
uint16_t local_id = NULL, remote_id = 0x6965; //0x691b;          // the network ID of the remote device
...
void setup() {
  ...
  Pozyx.setRangingProtocol(ranging_protocol, remote_id);   // set the ranging protocol
}
...
void rangeAndSerialPrint(uint16_t destination_id) {
  ...
  int status = Pozyx.doRemoteRanging(remote_id, destination_id, &range);
