...
////////////////// PARAMETERS //////////////////
////////////////////////////////////////////////
uint16_t destination_id1 = 0x6e49;
uint16_t destination_id2 = 0x691B;     // the network id of the other pozyx device: fill in the network id of the other device
...
uint16_t remote_id = 0x690e;          // the network ID of the remote device
...
void setup(){
  ...
  Serial.println("\tdestination_id1 (target device)");
  ...
  Pozyx.setLedConfig(led_config, destination_id1);
  Pozyx.setLedConfig(led_config, destination_id2);
...
void loop(){
  ...
  if(!remote)
    status = Pozyx.doRanging(destination_id1, &range);
  else
    status = Pozyx.doRemoteRanging(remote_id, destination_id1, &range);
  ...
  if (status == POZYX_SUCCESS){
    ...
    if (ledControl(range.distance,destination_id1) == POZYX_FAILURE){
