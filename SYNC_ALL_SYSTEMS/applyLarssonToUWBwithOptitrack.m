...
for nF = 1:num.Anchors
    ...
    alluwbtime = [alluwbtime; pozyx.data(nF).time];
end
...
%% Failed attempt to fillgaps the signal
% alluwbtime = table2array(alluwbtime);
% data.TimestampsUWB = sort(alluwbtime);
% data.DistancesUWB = NaN(length(alluwbtime),num.Anchors);
% for nF = 1:length(data.TimestampsUWB)
%     currentTimeStamp = data.TimestampsUWB(nF);
%     for nA = 1:num.Anchors % find time that belongs to anchor
%         idxCurrentTimeStamp = (table2array(pozyx.data(nA).time) == currentTimeStamp);
%         if any(idxCurrentTimeStamp)
%             break;
%         end
%     end
